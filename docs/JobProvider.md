...
public class InfoJobsProvider : IJobProvider
{
    ...
    private const string EnvApiKey = "LLM_INFOJOBS_API_KEY";
...
INSERT INTO job_providers (portal, provider_name, is_enabled, base_url, api_key) VALUES
    ('InfoJobs', 'Native', TRUE, 'https://api.infojobs.net/', NULL)
ON CONFLICT (portal, provider_name) DO NOTHING;
```

- `is_enabled = TRUE`: the provider will be called on every fetch.
- `api_key` can be `NULL` — the provider uses the env var instead.
...
```bash
INFOJOBS_API_KEY=your_key_here
```
...
| `401 Unauthorized` | API key missing in env var |

...
public partial class AiService
{
        ...
        List<string> allKeywords, CancellationToken ct = default)
    {
        try
        {
            var (systemPrompt, userTemplate, defaultModelId, useReasoning, reasoningEffort) = await LoadPromptAsync("dedup_keywords");
            ...
            var schema = LoadSchema("dedup_keywords", resolved.Provider.ServiceName);
            var result = await CallWithRetryAsync(resolved, systemPrompt, userPrompt, schema, "dedup_keywords", ct, useThinking: useReasoning, thinkingEffort: reasoningEffort);
